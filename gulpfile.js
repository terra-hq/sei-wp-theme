...
// Task to delete files in the dist folder that do not match the given hash
async function cleanHash(done) {
  var env = args.env;
  console.log('Environment:', env);
  var sftpConfig = getSFTPConfig(env);
  var hashToKeep = args.hash;

  if (!hashToKeep) {
    console.error("Error: No hash specified. Use --hash to specify the hash to keep.");
    done(new Error("No hash specified"));
    return;
  }

  console.log('Deleting files that do not match hash:', hashToKeep);
  ...
  try {
    ...
    let files = await sftp.list(sftpConfig.remotePath + '/dist');

    for (let file of files) {
      if (!file.name.includes(hashToKeep) && (file.name.endsWith('.css') || file.name.endsWith('.js'))) {
        try {
          await sftp.delete(sftpConfig.remotePath + '/dist/' + file.name);
          console.log('Deleted remote file:', file.name);
        } catch (err) {
          console.error('Error deleting remote file:', file.name, err);
        }
      }
    }
